...
void runFordJohnsonTests() {
  ...
  std::cout << std::endl
            << "Starting large-scale Ford-Johnson verification with "
            << COLOR_GREEN << "std::list " << COLOR_RESET << "..." << std::endl;
  for (size_t n : testSizes) {
    size_t maxAllowedComparisons = getFJComparisonLimit(n);
    ...
    for (int trial = 0; trial < 100; ++trial) {
      ...
      while (second != sorted.end()) {
        assert(first->value <= second->value && "List is not sorted!");
        ++first;
        ++second;
      }
      // Verify optimal comparisons
