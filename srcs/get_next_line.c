...
/*   Updated: 2025/07/25 17:32:00 by nraatika         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
#include <libft.h>
...
*/
static char	*make_str_gnl(t_data_gnl *d, ssize_t size)
{
	char	*s;
	ssize_t	len;
	ssize_t	i;
...
static char	*handle_new_index(t_data_gnl *d, ssize_t index)
{
	char	*temp;

	if (index == -1 && d->eof == BUFFER_SIZE)
	{
		temp = make_str_gnl(d, BUFFER_SIZE);
		d->start = BUFFER_SIZE;
	}
	else
	{
...
*/
static ssize_t	check_update_buffer(t_data_gnl *d, char **s)
{
	ssize_t	new_index;
	char	*temp;
	char	*new;
	...
	if (!temp)
		return (-2);
	new = ft_strjoin_gnl(*s, temp, ft_strlen(temp));
	free(temp);
	if (!new)
		return (-2);
	free(*s);
...
static long	read_until_line(t_data_gnl *d, char **s, int fd)
{
	ssize_t	rd;
	ssize_t	in;
...
{
	char				*ret;
	static t_data_gnl	d = {.start = BUFFER_SIZE, .eof = BUFFER_SIZE};
	ssize_t				tmp;
