...
> $N$ philosophers are sitting around a round table, each with a bowl of food in front of them.<br>
> They each have a fork to their right.<br>
> A philosopher can either think, or eat. <br>
> To eat, they must use the for on their right *and* the one on their left (which is another philosophers 'right' fork)<br>
> Design an algorithm that each philosopher can execute, that ensures every philosopher can eat regularly, without knowing the state of their neighboring philosophers<br>
