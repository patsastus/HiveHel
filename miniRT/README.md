...
We chose to make a minimal implementation of lighting, so we work with the simplest possible version: we do a second raytracing pass, this time from the intersection point to each light source and in that same direction to each object in the scene, to correctly calculate shadows. If the intersection point is not obscured by an object, we calculate a contribution of the light source as the cosine of the angle between the light vector and the surface normal at the intersection point: This means that a surface normal in the same direction as the light (0&deg; ) will contribute fully, while one at a 90&deg; angle (corresponding to shining a light along the surface of a plane) will contribute nothing.
